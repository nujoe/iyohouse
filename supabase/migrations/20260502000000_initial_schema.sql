...
BEGIN
    ...
    -- 3. Lock workshop for update to prevent race conditions
    SELECT capacity INTO v_capacity FROM public.workshops WHERE id = p_workshop_id FOR UPDATE;
    IF v_capacity IS NULL THEN
        RAISE EXCEPTION 'Workshop not found.';
    END IF;
    
    -- 4. Count active registrations (pending + confirmed)
...
CREATE OR REPLACE FUNCTION public.confirm_payment_registration(p_registration_id UUID, p_transaction_id TEXT, p_amount INTEGER)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Registration not found or already processed/expired.';
    END IF;

    -- 2. Record payment
    INSERT INTO public.payments (registration_id, amount, transaction_id, status)
    VALUES (p_registration_id, p_amount, p_transaction_id, 'success');
