...
2. IYOHOUSE의 작은 결제수단 모달이 열린다. NICEPAY 결제창과 같은 중립적인 흰 배경·얇은 테두리·검정 텍스트 톤으로 구성하고, 설명·아이콘·추가 선택지는 두지 않는다. 우측 상단에는 닫기 버튼 하나를 두고, 본문에는 큰 버튼 두 개만 둔다.
   - `카드·간편결제`
   - `가상계좌`
...
- 워크숍 상세: 기존 신청 버튼과 레이아웃은 유지한다. 클릭 시만 작은 결제수단 모달을 추가한다. 모달은 NICEPAY와 어울리는 중립적 카드이며, 우측 상단 닫기 버튼과 `카드·간편결제`, `가상계좌` 큰 버튼만 표시한다.
- NICEPAY 결제창: 카드·간편결제 및 가상계좌 처리 화면은 NICEPAY 기본 UI를 그대로 사용한다.
- 결제 완료·실패·입금 대기 페이지: 로그인 모달 프레임을 재사용하지 않고 NICEPAY와 같은 흰 배경·얇은 테두리·검정 텍스트·단순한 버튼 스타일로 통일한다. 입금 대기에는 계좌 정보, 금액, 입금 기한, `입금 후 신청이 확정됩니다`를 표시한다. 본인 주문만 조회한다.
