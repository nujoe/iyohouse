...
2. IYOHOUSE의 작은 결제수단 모달이 열린다. NICEPAY 결제창과 같은 중립적인 흰 배경·얇은 테두리·검정 텍스트 톤으로 구성하고, 설명·아이콘·추가 선택지는 두지 않는다. 큰 버튼 두 개만 둔다.
   - `카드·간편결제`
   - `가상계좌`
...
- 워크숍 상세: 기존 신청 버튼과 레이아웃은 유지한다. 클릭 시만 작은 결제수단 모달을 추가한다. 모달은 NICEPAY와 어울리는 중립적 카드이며, `카드·간편결제`와 `가상계좌` 큰 버튼만 표시한다.
