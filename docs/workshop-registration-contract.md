...
- **Payments 테이블 구조**: 
  - 테이블명: `payments`
  - 컬럼: `id`, `registration_id` (FK), `amount`, `payment_method`, `transaction_id` (UNIQUE), `status`, `created_at`
...
- **리다이렉트 URL**: 결제 요청 시 전달하는 `successUrl`과 `failUrl`에는 반드시 `registration_id` 파라미터가 포함되어야 합니다.
...
4. `test_sk_`로 시작하는 평문 토스 시크릿 키의 코드 내 하드코딩
...
1. **금지어 검사**: `rg "workshop_registrations[^_]|payment_status|'completed'|test_sk_"` 명령어를 통해 프로젝트 내 금지어가 하나도 발견되지 않아야 함.
